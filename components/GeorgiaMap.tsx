...
type City = { x: number; y: number; hub?: boolean };

const CITIES: City[] = [
  { x: 72.43, y: 74.2, hub: true },
  { x: 75.01, y: 80.83 },
  { x: 61.7, y: 63.41 },
  { x: 82.08, y: 66.03 },
  { x: 40.93, y: 52.08 },
...
export default function GeorgiaMap({ cities, mapAlt }: { cities: readonly string[]; mapAlt: string }) {
  ...
  return (
    <div ref={ref} className="relative mx-auto w-full max-w-4xl">
      <svg viewBox="0 0 1000 502" className="w-full" role="img" aria-label={mapAlt}>
        <defs>
